    ...
    <div className="p-6">
      ...
      <div className="flex gap-6 overflow-x-auto pb-6 pr-6">
        {STATUSES.map(status => (
          <div
            key={status.value}
            className="flex-shrink-0 w-96 glass rounded-2xl p-4 min-h-screen"
            onDragOver={handleDragOver}
            onDrop={(e) => handleDrop(e, status.value)}
          >
            <h3 className="font-semibold text-gray-800 mb-4 text-lg">{status.label}</h3>
            ...
            <div className="space-y-4">
              {getProjectsByStatus(status.value).map(project => (
                <div
                  ...
                  className="card p-5 cursor-grab active:cursor-grabbing hover:shadow-xl transition-all border border-transparent hover:border-green-300"
                >
                  ...
                  <div className="space-y-2 text-sm glass-sm p-3 rounded-xl my-3">
